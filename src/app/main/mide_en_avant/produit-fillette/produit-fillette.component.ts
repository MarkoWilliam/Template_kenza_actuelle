...
@Component({
  selector: 'app-produit-fillette',
  templateUrl: './produit-fillette.component.html',
  styleUrls: ['./produit-fillette.component.scss']
})
export class ProduitFilletteComponent implements OnInit {
  ...
  async ListeProduit() {
   ...
   await this.servGlobal.getAllFillette().subscribe(async results => {
...
let model={
  ...
  this.servGlobal.insertionFilette(model).subscribe(results => {
...
return new Promise((resolve) => {
  this.servGlobal.getProduitFille().subscribe(results => {
    this.liste = results.body;
    console.log("filette", this.liste )
